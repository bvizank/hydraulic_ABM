...
def read_comp_data(loc, read_list, days, truncate_list):
    ...
    for item in read_list:
        ...
        if item in truncate_list:
            x_len = days * 24
            out_dict["avg_" + item] = out_dict["avg_" + item].iloc[-x_len:]
            out_dict["var_" + item] = out_dict["var_" + item].iloc[-x_len:]
    ...
    for i, key in enumerate(data):
        ...
        if i != (len(data) - 1):
            for j in range(math.ceil(data[key] / 1000 * n_house)):
                income.append(
                    model.random.uniform(list(data.keys())[i], list(data.keys())[i + 1])
                )
        else:
            # at the end we need to arbitrarily set an upper bound
            for j in range(math.ceil(data[key] / 1000 * n_house)):
                income.append(
                    model.random.uniform(list(data.keys())[i], list(data.keys())[i] * 3)
                )
        index += data[key]
