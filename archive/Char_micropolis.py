...
data = pd.read_excel(r'Micropolis_pop_at_node.xlsx')
...
# Determening how many agents are going to be in every node at initializing at 00:00
# In total at industrial nodes = 1092 Agents, and at Residential =3514, none at commercial nodes

import random
Nodes_res_indust = Nodes_industr + Nodes_resident
Init_pop_res_indust = {}
f = Micro_pop

# Initializing Agents at residential nodes. They are distributed randomly between around 60 - 100 % of the capacity
# In some simulation "runs" it ends up leaving some nodes empty which can be reasoned with people at work or not in Town?
f_res = 3514
l = 0
for node in Nodes_resident:
    g = int(round(Max_pop_pnode_resid[node]*0.60))
    g_max = int(Max_pop_pnode_resid[node])
    l+=1
    # print(l)
    if g-f_res < 0:
        Init_pop_res_indust[node] = random.randint(g,g_max)
        f_res = f_res - Init_pop_res_indust[node]
    else:
        Init_pop_res_indust[node] = f_res
        break

sum_agents = sum(Init_pop_res_indust.values())
# print(sum_agents)


# Assigning Agents which are in Industrial Nodes, knowing that its at the max capacity
f_indust = f - f_res
p=0
for node in Nodes_industr:
    g_max = int(Max_pop_pnode_resid[node])
    p+=1
    # print(p)
    if g-f_indust < 0:
        Init_pop_res_indust[node] = g_max
        f_indust = f_indust - Init_pop_res_indust[node]
    else:
        Init_pop_res_indust[node] = f_res
        break
...
# Nodes at commercial nodes, Initialization:
Nodes_comm_all = Nodes_comm_cafe + Nodes_comm_rest + Nodes_comm_dairy

for node in Nodes_comm_all:
    Init_pop_res_indust[node] = 0
# print(len(Init_pop_res_indust))

#print(sum_agents)


#Import table for timesteps per nodetypes per hour
Nodes_distribution_ph = pd.read_excel(r'Micropolis_pop_at_node_kind.xlsx')
...
Cleared_nodes_iterations = pd.read_excel(r'Cleared_node_names.xlsx')
...
Sleep_data = pd.read_excel(r'sleep_data.xlsx')
Sleep_distr = Sleep_data['sleep_data'].tolist()

radio_data = pd.read_excel(r'Radio_data.xlsx')
radio_distr = radio_data['radio_data'].tolist()

TV_data = pd.read_excel(r'TV_data.xlsx')
