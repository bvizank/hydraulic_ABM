...
class BaseGraphics:
    ...
    def package_household(self, data, dir, bw=True):
        ...
        for i in range(30):
            ...
            data["income"].loc[data["income"]["i"] == i, "income"] = curr_income[
                0 : len(data["income"][data["income"]["i"] == i])
            ]

            # set the level as 1 if above 20 %-tile and 0 if below
            bot20 = np.percentile(
                data["income"].loc[data["income"]["i"] == i, "income"], 20
            )
            data["income"].loc[data["income"]["i"] == i, "level"] = np.where(
                data["income"].loc[data["income"]["i"] == i, "income"] > bot20, 1, 0
            )
    ...
    def make_income_comp_plot(
        ...
        if box:
            ...
            axes[0].text(
                0.5, text_y, "(a)", size=12, ha="center", transform=axes[0].transAxes
            )
            axes[1].text(
                0.5, text_y, "(b)", size=12, ha="center", transform=axes[1].transAxes
            )
...
class Graphics(BaseGraphics):
    ...
    def flow_plots(self):
        ...
        for col in main_pipes.columns:
            ...
            first30 = (
                main_pipes.head(720).loc[:, col].mean() / 1000000 / pipe.diameter**2
            )
            last30 = (
                main_pipes.tail(720).loc[:, col].mean() / 1000000 / pipe.diameter**2
            )
            if first30 != 0.0:
        ...
        wntr.graphics.plot_network(
            self.wn,
            link_attribute=main_dict,
            link_colorbar_label="% Change Vel.",
            link_range=[-200, 200],
            node_size=0,
            link_width=2,
            link_cmap=mpl.colormaps["PRGn"],
        )
        plt.gcf().set_size_inches(7, 5.5)
        plt.savefig(
            self.pub_loc + "velocity_changes_main_pipes." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.ylabel("Flow (gpm)")
        plt.savefig(
            self.pub_loc + "MA1066_flow." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def demand_plots(self):
        ...
        var_pm = self.pm["var_demand"][self.res_nodes + self.com_nodes + self.ind_nodes]
    ...
    def age_plots(self):
        ...
        plt.savefig(
            self.pub_loc + "tank_age." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        age_pm = self.pm["avg_age"][self.res_nodes + self.com_nodes + self.ind_nodes]
        ...
        var_pm = self.pm["var_age"][self.res_nodes + self.com_nodes + self.ind_nodes]
        ...
        axes = self.make_avg_plot(
            ...
            ["Base", "TWA", "PM", "TWA+PM"],
            self.x_values_hour,
            fig_name="system_water_age",
            xlabel="Time (days)",
            ylabel="Water Age (hours)",
            show_labels=True,
        )
        plt.savefig(
            self.pub_loc + "system_water_age" + "." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        axes[0, 0] = self.make_avg_plot(
            axes[0, 0], age_base / 3600, age_base_err / 3600, cols, self.x_values_hour
        )
        ...
        axes[0] = self.make_avg_plot(
            axes[0], age_base / 3600, age_base_err / 3600, cols, self.x_values_hour
        )
        axes[1] = self.make_avg_plot(
            axes[1], age_basebw / 3600, age_basebw_err / 3600, cols, self.x_values_hour
        )
        ...
        plt.savefig(
            self.pub_loc + "mean_age_sector_nopm." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        data = {"low": age_low, "high": age_high}
    ...
    def cowpi_boxplot(self, di=False, perc=False, exclusion=False, sa=False):
        ...
        plt.savefig(
            self.pub_loc + "cowpi_all." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        if sa:
            cowpi_low = [
                self.pm["cowpi"][self.pm["cowpi"]["level"] == 0]["cowpi"] * 100,
                self.pm25ind["cowpi"][self.pm25ind["cowpi"]["level"] == 0]["cowpi"]
                * 100,
                self.pm50ind["cowpi"][self.pm50ind["cowpi"]["level"] == 0]["cowpi"]
                * 100,
                self.pm75ind["cowpi"][self.pm75ind["cowpi"]["level"] == 0]["cowpi"]
                * 100,
                self.pm100ind["cowpi"][self.pm100ind["cowpi"]["level"] == 0]["cowpi"]
                * 100,
            ]

            cowpi_high = [
                self.pm["cowpi"][self.pm["cowpi"]["level"] == 1]["cowpi"] * 100,
                self.pm25ind["cowpi"][self.pm25ind["cowpi"]["level"] == 1]["cowpi"]
                * 100,
                self.pm50ind["cowpi"][self.pm50ind["cowpi"]["level"] == 1]["cowpi"]
                * 100,
                self.pm75ind["cowpi"][self.pm75ind["cowpi"]["level"] == 1]["cowpi"]
                * 100,
                self.pm100ind["cowpi"][self.pm100ind["cowpi"]["level"] == 1]["cowpi"]
                * 100,
            ]
    ...
    def make_single_plots(self, file, days):
        ...
        plt.savefig(
            loc + "mean_res_demand" + "." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
        ...
        self.make_avg_plot(
            ax,
            pm_age.iloc[-x_len:] / 3600,
            pm_age_sd[-x_len:] / 3600,
            cols,
            x_values,
            "Time (days)",
            "Water Age (hr)",
        ...
        plt.savefig(
            loc + "_sector_age." + self.format, format=self.format, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.gcf().set_size_inches(7, 3.5)
        plt.savefig(
            loc + "income_map." + self.format, format=self.format, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.savefig(
            loc + "tot_cost_map." + self.format, format=self.format, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.savefig(loc + "twa." + self.format, format=self.format, bbox_inches="tight")
        plt.close()

        """ Equity metric costs """
        metrics = pd.concat(
            [data["traditional"], data["burden"]],
            axis=1,
            keys=["Traditional", "Burden"],
        )
        ...
        plt.savefig(
            loc + "equity_metrics." + self.format,
            format=self.format,
            bbox_inches="tight",
        )
        plt.close()
